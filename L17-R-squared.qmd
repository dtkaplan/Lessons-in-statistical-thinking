...
```{r echo=FALSE, digits = 3, and_so_on="... for 154 coefficient altogether"}
Race_name_model1 |> conf_interval() |> head() |> kable()
```
...
Adding in `race` increases the R^2^ by only three percentage points, to 95%. (See exercise `r reference_exercise("Q30-201")`.)
