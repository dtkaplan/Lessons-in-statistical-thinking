...
A p-value can be calculated for any `r gdef("sample statistic")` by the shuffling method. There are also formulas for them when dealing with `r gdef("R^2^", "def-r-squared")`:
...
```{r echo=FALSE, and_so_on = "... for 500 trials altogether.", digits=2, results=asis()}
Sim_results |> head(5) 
```
