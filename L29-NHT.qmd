...
This confusing situation could be avoided entirely by switching from "significant" to a word that conveys the correct meaning. For example, statistician [Jeffrey Witmer](https://www.tandfonline.com/doi/full/10.1080/10691898.2019.1702415) has proposed the word "discernible" be used in place, as in, "The difference between groups is statistically discernible," or, "We found a discernible difference." 
...
```{r label='520-NHT-2upeyz', echo=FALSE, and_so_on="... for 200 men altogether"}
set.seed(102)
Height_data <- tibble::tibble(
  height = sample(Galton$father, n=200),
  nationality = sample(c("English", "French"), n=200, replace = TRUE)
)
Height_data |> head()
```
...
```{r label='520-NHT-H4GOtg', digits=2}
Height_data |> 
...
```{r echo=FALSE, and_so_on = "... for 500 trials altogether.", digits=2}
Sim_results |> head(5) |> kable()
```
...
In the voting simulation with n=1000, the power is about 7%. That's very small compared to the target power of about 80%. In `r reference_exercise("Q29-201")` you can explore how big a sample size is needed to reach 80%.
