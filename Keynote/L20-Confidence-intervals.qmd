...
Lesson [-@sec-sampling-variation] took a simulation approach to observing sampling variation: generate many trials from a source such as a DAG and observe how the same sample statistic varies from trial to trial. We quantified the sampling variation in the same way we usually quantify variation, taking the **variance** of the sample statistic across all the trials. We called this measure of variation the **sampling variance** as a reminder that it comes from repeated trials of sampling. 
...
```{r label='375-Confidence-intervals-wo310S', digits = 3}
Hill_racing |> 
  model_train(time ~ distance + climb) |> 
  conf_interval() |>
...
A more complete picture of sampling variation is provided by treating it as a noise model, as described in Lesson [-@sec-noise-patterns]. We can choose an appropriate noise model by looking at the distribution shape for sampling variation. Experience has shown that an excellent, general-purpose noise model for sampling variation is the *normal* noise model. To support this claim we can use a simulation of the sort reported in @fig-sampling-distribution, where the distribution of coefficients across the 500 sampling trials has the characteristic shape of the normal model.
...
```{r label='375-Confidence-intervals-z9z2RX', cache=TRUE}
Trials <- 
  resample(Hill_racing) |>
  model_train(time ~ distance + climb) |>
  conf_interval() |>
...
```{r label='375-Confidence-intervalsgWgaoo'}
TenMileRace |> 
  filter(age > 40) |>
  model_train(net ~ age) |> 
  conf_interval()
...
```{r label='375-Confidence-intervals-0weHQz'}
...
```{r label='375-Confidence-intervals-cmkZUU', digits=2}
