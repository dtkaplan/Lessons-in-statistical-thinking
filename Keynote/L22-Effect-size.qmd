...
```{r label='430-Effect-size-wJhbMh'}
...
```{r label='430-Effect-size-Lig3VO'}
...
```{r label='430-Effect-size-klARxv'}
...
```{r label='430-Effect-size-xaNcXu'}
...
```{r label='430-Effect-size-vqIzZY'}
...
```{r label='430-Effect-size-3nwfET'}
...
```{r label='430-Effect-size-5V8vzC'}
Hill_racing |> model_train(time ~ distance * climb) |> conf_interval()
```
