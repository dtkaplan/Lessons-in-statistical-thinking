...
::: {.column-margin #tbl-bed-net style='width:70%;'}
```{r echo=FALSE}
Bed_net_data |> knitr::kable(align="r") |>
...
```{r label='462-ExperimentbixY4O', digits=3}
Bed_net_data |> model_train(reduction ~ 1) |> 
  conf_interval()
```

```{r label='462-ExperimentiDuRlP', digits=3}
Bed_net_data |> model_train(reduction ~ 1) |> 
  conf_interval()
```
...
::: {.column-margin #tbl-bed-net-controls style="text-align: center;"}
```{r echo=FALSE}
...
`With_controls`, data from a new study where five sites were used as controls.
:::

The proper regression model for the `With_controls` data is `reduction ~ nets`:

```{r label='462-Experiment-anihSs', digits=2}
With_controls |> 
