...
Lesson [-@sec-sampling-variation] took a simulation approach to observing sampling variation: generate many trials from a source such as a DAG and observe how the same `r gdef("sample statistic")` varies from trial to trial. We quantified the sampling variation in the same way we usually quantify variation, taking the `r gdef("**variance**")` of the sample statistic across all the trials. We called this measure of variation the `r gdef("**sampling variance**")` as a reminder that it comes from repeated trials of sampling. 
