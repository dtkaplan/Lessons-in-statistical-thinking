...
@fig-mass-bill-species

@def-data-frame

`r glossary_word("data frame", "def-data-frame")`

`r glossary_word("variable", "def-variable")`
...
To illustrate, we'll return to a historical statistical landmark, the data collected in the 1880s by Francis Galton on the heights of parents and their full-grown children. In that era, there was much excitement, uncertainty, and controversy about Charles Darwin's *theory of evolution*. 
...
@lst-galton-mean shows how to calculate the average `height` for each `sex` using the techniques from Lesson [-@sec-wrangling]. 

::: {#lst-galton-mean}
```{r digits=6, raw=TRUE, results = "hide"}
Galton |>
  summarize(       # <1> 
...
This simple report indicates that the sexes differ, on average, by about 5 inches in height. But this report offers no insight into how much of the person-to-person variation in height is attributable to `sex`. For that, we need to look at the group averages in the context of the individuals. That is, instead of computing the model value for each `sex`, we will compute it for each specimen using `mutate()`. Even so, the model value will remain the mean for each sex. 

::: {#lst-galton-mutate}
```{webr-r}
Galton |> 
  select(father, mother, sex, height) |>
  mutate(modval = mean(height), .by = sex) |>
  sample(n = 3, .by = sex)
```
:::

The last line of the calculation in @lst-galton-mutate simplifies the display of the output; Without this line, the output would be 898 rows long! Instead, we show three random specimens for each `sex`. The `select()` in the select line also is there to simplify the output, discarding the variables we don't need for the example.

Looking at the output from @lst-galton-mutate, you can see that the model values (`modval`) are the same for all the females, but different from the model value for the males.

@lst-galton-mutate2 repeats the above calculation, but rather than displaying 898 rows, the result is summarized:
...
{{< include LearningChecks/L10/LC10-01.qmd >}}

{{< include LearningChecks/L10/LC10-02.qmd >}}
...
{{< include LearningChecks/L10/LC10-03.qmd >}}
