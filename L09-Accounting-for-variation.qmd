...
::: {#tbl-galton-model1d .column-margin .column-margin .content-visible when-format="html"}
```{r digits=4, results="asis"}
#| code-fold: true
<<make-model1>>
set.seed(101)
...
The residual is the difference between the response variable (`height`) and the model values.
:::


```{r make-model1, results=asis()}
Model1 <- Model1 |>
...
```{r echo=FALSE, digits=2, and_so_on = "... for 898 rows altogether.", results=asis()}
set.seed(183)
...
```{r label='210-Prelude-to-modeling-RTjfOt', digits=c(4,2,4), results=asis()}
Model3 |>
  summarize(var(height), var(modval), var(resid))
```
