...
Learning command patterns is a powerful enabler in statistical thinking. The computer is an indispensable tool for statistical thinking. (Enrichment [-@nte-computing-history] gives a little history of the long-standing link between statistics and computing.) In addition to doing the work of calculations, computer software is a medium for describing and sharing with others statistical techniques and concepts. For instance, three fundamental operations in statistics---randomize, repeat, collect---are easily written in computer languages but have no counterpart in the algebraic notation traditionally uused in mathematics education. 
...
:::: { #enr-displaying-tables .callout-note collapse="true"}
## Displaying tables 
...
Growth_rate |> mutate(yearly_growth = as.character(signif(yearly_growth, digits=15))) |> data.frame() 
```
